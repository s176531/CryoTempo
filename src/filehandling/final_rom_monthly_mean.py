...
def main(station: str = "soberania"):
    """Compute the monthly average of CryoTEMPO phase 2.2 data"""
    data = load(Path("data", "po_tds_c_final"), Path("south_combined.nc"))

    deg_size = .5
    if station == "soberania":
        lat, lon = -62.483333+4*deg_size, -59.633333
    elif station == "syowa":
        lat, lon = -69.0125+deg_size, 39.59
    else:
        raise ValueError("Input station must be 'soberania' or 'syowa'")
    ...
    all_averages.to_netcdf(Path(f"data/po_tds_c_final/monthly_averages_{station}.nc"))
